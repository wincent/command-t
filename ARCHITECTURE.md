...
The other significant difference is that exec finders scan _asynchronously_: rather than blocking the editor until the external command (`fd`, `git-ls-files`, `rg`, `find`, and friends) finishes enumerating what could be hundreds of thousands or millions of files, they fork the command and read its output on a background thread while the UI stays responsive, streaming matches in as candidates arrive. The benchmark suite uses that same asynchronous scanner (`scanner_new_exec_async()`), followed by a blocking `scanner_wait()`; the exec finder instead polls for completion without blocking the editor. See the "Streaming and concurrency" section for the full picture. (The built-in "file" and watchman finders remain synchronous.)
...
- **Scanners** manage access to a list of haystacks to be searched. The come in four varieties:
  1. Scanners created by `scanner_new_exec()` (synchronous) or `scanner_new_exec_async()` (asynchronous), which obtain their candidates by running commands like `git-ls-files`, `fd`, `find`, `rg`, and friends. The exec finder (`finders/exec.lua`) uses the asynchronous constructor, which produces candidates on a background thread (see "Streaming and concurrency"). The benchmark's "exec" scanner (`scanners/exec.lua`) uses the same constructor and blocks in `scanner_wait()`. The synchronous constructor remains as a compatibility wrapper around those two calls, so all exec scanners share one producer implementation and own the same `candidates` and `buffer` slabs.
...
This is the most common form of scanning in Command-T, used by anything that wraps an external command (eg. `:CommandTGit`, wrapping `git-ls-files`, `:CommandTRipgrep`, wrapping `rg`, and so on). Both constructors use the same background producer and memory-ownership model. `scanner_new_exec_async()` returns without waiting for production to finish; `scanner_new_exec()` is a blocking compatibility wrapper that waits before returning. The exec finder uses the asynchronous constructor directly, while the benchmark adapter calls it followed by `scanner_wait()`; see "Streaming and concurrency" for the threading details.
...
- The main controller (defined in `init.lua`) calls `finders.exec()` (defined in `finders/exec.lua`) to obtain a `finder`:
  - `finders.exec()` calls `lib.scanner_new_exec_async()`, which calls `commandt_scanner_new_exec_async()` via FFI:
    - That function creates a `candidates` slab and a `buffer` slab with `xmap()`. The former stores zero-copy `str_t` records (created with `str_init()`), while the latter holds the actual command output, into which the `str_t` records index via their `contents` pointers. It then forks the command and spawns the producer thread that fills those slabs. (The blocking `scanner_new_exec()` wrapper waits for that producer and cleans up before returning.)
  - `lib.scanner_new_exec_async()` uses `ffi.gc()` to mark the returned `scanner` object such that when it is garbage-collected, the `commandt_scanner_free()` function will be called:
    - `commandt_scanner_free()` first calls `commandt_scanner_stop()` to join the producer thread and reap the child (a no-op for the completed blocking wrapper and the non-exec scanner kinds), then uses `xmunmap()` to release the `candidates` and `buffer` slabs, and `free()` to deallocate the `scanner_t` struct itself.
...
`commandt_scanner_new_exec_async()` (in `scanner.c`, wrapped by `lib/scanner_new_exec_async.lua`) allocates the `candidates` and `buffer` slabs, then forks the command and returns immediately, spawning a background "producer" thread (`exec_producer()`) that owns the read loop. The blocking compatibility constructor uses this same implementation. The producer blocking-reads the child's stdout into the `buffer` slab and tokenizes it into zero-copy `str_t` records in the `candidates` slab (via the shared `scanner_tokenize()` helper), publishing its progress by advancing the candidate `count`.

The slabs are large, sparse `mmap()` reservations that are never resized or moved, so production is purely append-only: the producer writes `candidates[count]` and then advances `count`. That single field, `scanner->count`, is the entire synchronization surface between the producer and the consumer. The producer publishes it with a release store (`__atomic_store_n(..., __ATOMIC_RELEASE)`) after fully writing a batch of `str_t` records; C consumers use the inline `scanner_count_snapshot()` helper to acquire-load a fixed count; Lua callers use its exported `commandt_scanner_count_snapshot()` wrapper. The matcher iterates only over that snapshot, rather than repeatedly reading the live count. A plain count read is safe for eager scanners or after completion has been synchronized, for example by `scanner_done()` returning true or by `scanner_wait()` or `scanner_stop()` joining the producer. Because the slabs never move and every entry below `count` is immutable once published, no locks are required, and the release/acquire pair guarantees that a consumer which observes a given `count` also observes the fully-initialized candidates below it.
...
The scanner exposes three lifecycle entry points for the async case:
...
- `commandt_scanner_wait()`: blocks until the producer finishes, then calls `scanner_stop()` for the same cleanup the UI performs on completion. It joins before signalling, so it does not cancel an in-progress scan. If a command closes stdout but keeps running, the remaining processes are terminated: completion means the scanner has finished producing, not that the command has finished all its other work. Like `scanner_stop()`, it is idempotent, a no-op for non-async scanners, and must run on the owning thread without concurrent lifecycle calls. Benchmarks use it; the UI must not block on it.
...
The scanner benchmark's exec adapter is blocking, but starts the same async producer as the UI and waits for it in C without polling. Its timed region includes thread creation, scanning, joining, and process cleanup, so its timings are not directly comparable to older runs using the separate synchronous read loop. The blocking `scanner_new_exec()` API remains as a compatibility wrapper, not a separate scanning implementation. The built-in "file" finder (`find.c`, `commandt_file_scanner()`) still walks the filesystem synchronously on the main thread, which is also why the exec finders' fallback to it can briefly block on a cold, large tree; making the file finder stream as well is a natural future extension[^future]. The watchman finder receives its candidates as a single in-memory payload, so it has nothing to stream.
