...
lib  = File.expand_path('../../ruby/command-t/lib', File.dirname(__FILE__))
$LOAD_PATH.unshift(lib) unless $LOAD_PATH.include?(lib)

require 'command-t'
require_relative '../../ruby/command-t/ext/command-t/ext'
require 'benchmark'
require 'yaml'


class CommandT::Benchmark
  DIFFERENCE = 0
  ABSOLUTE = 1
  SIGN = 2
  ...
  def exec bench
    raise "Subclass must override #exec"
  end
  ...
  def run
    ...
    results = ENV.fetch('TIMES', self.class::TIMES).to_i.times.map do
      Benchmark.bmbm do |b|
        exec b
      end
    end
