...
WILD_IGNORE = ENV.fetch('DIR', '*.o,*.pyc')
results = TIMES.times.map do
  Benchmark.bmbm do |b|
    scanner = CommandT::Scanner::FileScanner.for_string(TYPE).new DIR,
        wild_ignore: WILD_IGNORE,
        max_files: 1e9
    b.report do
      scanner.flush
      print scanner.paths.length
    end
  end
end
